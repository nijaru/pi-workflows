...
| Feature | Behavior |
|---------|----------|
| Agents | `agent(prompt, opts)` fails the workflow on agent failure; SDK truncation, unfinished tool requests, empty results, and write workers with no tool activity are never silently returned as success. |
...
| Effects | Agent calls default to `effect: "write"`; concurrent canonical writes (within or across runs) require `isolation: "worktree"`. Host Pi `edit`/`write` calls are blocked while a canonical workflow mutation holds the workspace lock. `effect: "read"` uses the built-in read-only tool allowlist for safe fan-out. |
...
| Resume | Results are reused only when script, args, execution policy, active model/thinking identity, research backend, Git state, and dirty-file contents match. Automatic matching is restricted to runs created by the current Pi session; use an explicit `runId` to adopt a run from another session. Resume is at-least-once around external side effects. |
...
| `/workflows pause <runId>` | Pause at the next workflow or agent boundary, including calls waiting for concurrency admission. |
...
| `resume` | `true` | Reuse the newest matching incomplete run created by the current Pi session. Use `runId` for explicit cross-session adoption. |
